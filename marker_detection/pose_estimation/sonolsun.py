...
def compute_essential_matrix(x1, x2, return_sigma=False):
    """
    ...
    assert x1.shape[1] in [3, 4], f"PCL out of shape {x1.shape} != (3, n) or (4, n)"

    A = building_matrix_A(x1, x2)
...
def recover_pose_from_e(E, x1, x2):
    transformations = get_the_four_cam_solutions_from_e(E, x1, x2)
    return select_camera_pose(transformations, x1=x1, x2=x2)
    # return transformations


def select_camera_pose(transformations, x1, x2):
    """
    ...
    for idx, M in enumerate(transformations):
        pt1_3d = triangulate_points_from_cam_pose(cam_pose=M,
                                                  x1=x1,
                                                  x2=x2)
...
#     return e / np.linalg.norm(e)
#
# def recover_pose_from_matches(, x1, x2):
#     """
#     Returns the a relative camera pose by using LSQ method (Higgins 1981)
#     """
#     e = .compute_essential_matrix(x1, x2)
#     return .recover_pose_from_e(e, x1, x2)
#
